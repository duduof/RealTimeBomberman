...
static  FS_NAND_FREE_SPARE_DATA  App_FS_NAND_FreeSpareMap[] = {{ APP_CFG_FS_NAND_FREE_SPARE_START, APP_CFG_FS_NAND_FREE_SPARE_LEN},
                                                               {-1,-1}};
...
static  CPU_BOOLEAN  App_FS_AddNAND (void)
{
    ...
    FS_NAND_CTRLR_GEN_SOFT_ECC_CFG  soft_ecc_cfg = FS_NAND_CtrlrGen_SoftEcc_DlftCfg;
    ...
    soft_ecc_cfg.ECC_ModulePtr = &Hamming_ECC;
                                                                ...
                                                                /* ------------------ CFG NAND PART ------------------- */
#if   (APP_CFG_FS_NAND_PART_TYPE == ONFI)
    part_cfg.FreeSpareMap      = App_FS_NAND_FreeSpareMap;
#elif (APP_CFG_FS_NAND_PART_TYPE == STATIC)
    ...
    switch (err) {
        ...
        case FS_ERR_DEV_INVALID_LOW_FMT:                        /* Low fmt invalid (see Note #1b).                      */
             FSDev_IO_Ctrl("nand:0:", FS_DEV_IO_CTRL_CHIP_ERASE, DEF_NULL, &err);
             if (err != FS_ERR_NONE) {
                APP_TRACE_DBG(("    ...low-level chip erase failed.\r\n"));
                return (DEF_FAIL);
             }
